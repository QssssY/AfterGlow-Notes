...
从设计抄值的时候这一点特别舒服：量出来什么色号，粘进 `@theme` 就完事，不用在 JS 对象和 CSS 之间来回倒手。
...
设计给的是 22px 就写 `rounded-[22px]`，是 13.5px 就写 `text-[13.5px]`。4 的引擎按需生成，任意值不再有"撑大产物"的心理负担。一比一还原设计的场景里，这比凑近似的预设档位诚实得多。
