...
## 第 2 步 · 打开管理台（没有服务器也有管理台）

打开 `你的博客地址/overview` —— 无服务器形态下管理台走 **GitHub 直连模式**：
粘贴一个 GitHub 令牌登录，之后写文章、改站点数据、传图、调配色都在网页界面里点点就行，
**每次保存就是一次 git 提交**，托管平台自动重建，1~3 分钟生效。

登录用的令牌这样生成（一次性设置，约 1 分钟）：
...
4. 生成后复制（只显示一次），粘贴进 `/overview` 登录页。仓库一栏在 Vercel /
   GitHub Pages 部署时通常已自动填好，空着就手填 `你的用户名/仓库名`

> 令牌只存浏览器本会话（关浏览器即忘，换设备重贴一次），且只能读写你勾的那一个仓库。
> 管理台里「统计曲线 / 在线人数 / 友链巡检 / 音乐上传」需要服务器形态（页面里有说明），
> 其余功能全部可用。

### 备选：GitHub 网页直接改文件

不想用管理台也行：博客完全由仓库文件生成，在 GitHub 网页上打开文件 → 点右上角
**铅笔图标**（Edit）→ 改完 **Commit changes**，平台一样自动重建。各文件管什么（都在 `src/` 下）：
...
最顺手的是管理台编辑器：`/overview` 登录后点**「写新文章」**——所见即所得的字段、
标签胶囊、粘贴 / 拖拽直接传图、还能导入现成的 .md 文件，保存即发布。
不用管理台的话，GitHub 网页手工建文件也一样：
...
- **管理台登录不上？** 仓库要写成 `owner/repo`；令牌无效或过期就照第 2 步重新生成一个
  （必须勾 Contents: Read and write）。统计页显示「需要服务器形态」是正常的 ——
  那部分数据只有 Go 服务能收集，其余页签不受影响。
