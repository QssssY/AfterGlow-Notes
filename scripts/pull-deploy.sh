...
# 6.6MB 的 dist.tgz 要 ~13 分钟，240 秒**结构上**下不完 —— 不是运气不好，是每次
# 必然被掐。改成 nginx send_timeout 那种语义：不设总时长上限，但连续 60 秒慢过
# 2KB/s 就判它死了 —— 慢而通的链路给足时间，真断了立刻放手。1800 秒兜底。
# 地板取 2KB/s 而不是 8KB/s：实测这条线路整程就在 8~33KB/s 之间晃（一次完整
# 6.6MB 用了 13 分钟，均速约 8.5KB/s），阈值贴着均速放等于自己给自己下绊子。
#
...
dl() {
  curl -fsSL -C - --speed-limit 2048 --speed-time 60 --max-time 1800 \
